...
41. **Settings 用量页采用 `sessions.list` + 1 次 unscoped `sessions.usage`（最近 90 天 + `limit` 500）+ 客户端按可见 key 过滤；不再 per-key 循环。** openclaw `2026.4.5` 的 unscoped `sessions.usage` 已经返回每个 session 的 `usage.input/output/cacheRead` 与全局 `totals`，所以 per-key 批量循环纯粹是重复网关已经做完的事。但 unscoped 路径会调用 `discoverAllSessionsForUsage({ startMs, endMs })`，时间窗必须收紧到最近 90 天，避免 `startDate: "1970-01-01"` 触发全量历史扫描。可见性过滤改成客户端：先用 `sessions.list` 拿可见 key 集合（store 视角，归档项已被剔除），再用该集合过滤 `sessions.usage` 的行，并在过滤后的行上重新求和——不要直接采用网关返回的 `response.totals`（含归档份额，见 #42）。

42. **`sessions.usage`（不带 `key`）会把 `.jsonl.reset.*` / `.jsonl.deleted.*` 归档文件算进结果。** openclaw `2026.4.5` 的 `isUsageCountedSessionTranscriptFileName`（`src/config/sessions/artifacts.ts:37-42`）把这两类归档视为有效 usage 来源（仅排除 `.bak`），会以合成 key `agent:<agentId>:<sessionId>`、`label: undefined` 出现在响应里。`response.totals` 也含其份额。任何不希望显示「已归档/已重置」会话的 UI，必须先用 `sessions.list` 拿可见 key 集合（reset/delete 时 store 已 `delete store[canonicalKey]`，列表干净），再客户端过滤行 + 重新求和；不能直接采用 `response.totals`。
